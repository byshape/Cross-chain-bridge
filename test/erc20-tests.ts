...
describe("Token20", () => {
    ...
    it("Deploys token contract", async () => {
        ...
        await erc20.grantRole(BURNER_ROLE, admin.address);
    });
    ...
    it("Does not burn tokens from the zero address", async () => {
        ...
        await expect(erc20.connect(admin).burn(zeroAddress, amountToBurn)).to.be.revertedWith("Burn from the zero address");
    ...
    it("Does not burn tokens more than balance", async () => {
        ...
        await expect(erc20.connect(admin).burn(tokensOwner.address, amountToBurn)).to.be.revertedWith("Burn amount exceeds balance");
    ...
    it("Burns tokens", async () => {
        ...
        expect(await erc20.balanceOf(tokensOwner.address)).to.be.equal(initialSupply);
        await expect(erc20.connect(admin).burn(tokensOwner.address, amountToBurn)).to.emit(erc20, "Transfer").withArgs(tokensOwner.address, zeroAddress, amountToBurn);
